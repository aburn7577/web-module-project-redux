    ...
    return (<div className="table-title">
        <div className="row">
            ...
            <div className="col-sm-6 headerBar">
                <div className="btn btn-sm btn-primary"><span>{displayFavorites ? (<input
                    className="btn btn-sm btn-primary"
                    type='button'
                    value='Hide Favorites'
                    onClick={handleFavorites} />) : (<input
                        className="btn btn-sm btn-primary"
                        type='button'
                        value='Show Favorites'
                        onClick={handleFavorites} />)}</span></div>
                <Link to="/movies"
                    className="btn btn-sm btn-primary">
                    View All Movies</Link>
                <Link to="/movies/add"
                    className="btn btn-sm btn-success"><i className="material-icons">&#xE147;</i> <span>Add New Movie</span></Link>
            </div>
        </div>
    </div>);
}
